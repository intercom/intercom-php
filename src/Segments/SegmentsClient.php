...
namespace Intercom\Segments;

use GuzzleHttp\ClientInterface;
use Intercom\Core\Client\RawClient;
use Intercom\Segments\Requests\ListSegmentsRequest;
use Intercom\Types\SegmentList;
...
use Intercom\Segments\Requests\FindSegmentRequest;
...
class SegmentsClient
{
     ...
     */
    public function list(ListSegmentsRequest $request = new ListSegmentsRequest(), ?array $options = null): SegmentList
    {
     ...
     *
     * @param FindSegmentRequest $request
     * @param ?array{
     *   baseUrl?: string,
     *   maxRetries?: int,
     *   timeout?: float,
     ...
     * @throws IntercomApiException
     */
    public function find(FindSegmentRequest $request, ?array $options = null): Segment
    {
        $options = array_merge($this->options, $options ?? []);
        try {
            $response = $this->client->sendRequest(
                new JsonApiRequest(
                    baseUrl: $options['baseUrl'] ?? $this->client->options['baseUrl'] ?? Environments::UsProduction->value,
                    path: "segments/{$request->getSegmentId()}",
                    method: HttpMethod::GET,
                ),
                $options,
            );
